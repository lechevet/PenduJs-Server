...
describe('/getRegister/{register}', () => {
    ...
    describe('GET', () => {
        ...
        it('should throw an error if there is no pending register', async function(): Promise<void> {
            await request(app)
                .get(config.application.api.basePath + '/auth/getRegister/1234567890abcdef12345678')
                .set('Authorization', 'Bearer ' + jwtAdmin)
                .expect(404)
                .expect((res) => {
                    expect(res.error).to.not.equal(null);
                    expect(res.error.message)
                    .to.equal(`cannot GET ${config.application.api.basePath}/auth/getRegister/1234567890abcdef12345678 (404)`);
