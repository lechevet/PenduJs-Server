...
describe('/validateRegister/{register}', () => {
    ...
    describe('PUT', () => {
        it('should validate one register', async function(): Promise<void> {
            await login(config.users.default.admin.login, 'test', 'Administrator', 'valid');
            await newPendingRegister('user@amiltone.com', 'test');
            await request(app)
                .put(config.application.api.basePath + '/auth/validateRegister/' + idUser)
        ...
        it('should throw an error if there is no pending register', async function(): Promise<void> {
            await request(app)
                .put(config.application.api.basePath + '/auth/validateRegister/1234567890abcdef12345678')
                .set('Authorization', 'Bearer ' + jwtAdmin)
                .expect(404)
                .expect((res) => {
                    expect(res.error).to.not.equal(null);
                    expect(res.error.message)
                    .to.equal(`cannot PUT ${config.application.api.basePath}/auth/validateRegister/1234567890abcdef12345678 (404)`);
                    const jsonError = JSON.parse(res.error.text);
                    expect(jsonError).to.deep.equal({
                      errorCode: 40401,
                      errorDetails: {},
                      statusCode: 404,
                      errorMessage: 'Can\'t find the requested information: Non existing register'
                    });
                });
        });
    });
